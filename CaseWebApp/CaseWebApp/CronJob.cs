...
namespace CaseWebApp
{
    public class CronJob : BackgroundService
    {
        ...
        public CronJob(DataContext dbContext)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            do
            {
                ...
                if (now > _nextRun)
                {
                    //10 saniyede bir bu tekrarlanır.
                    Process();
                    _nextRun = _schedule.GetNextOccurrence(DateTime.Now);
                }
        ...
        public async Task<IActionResult> Process()
        {
            ...
            var urlListData = db.URLList.Where(q => q.IsActive == true).ToList();
            ...
            foreach (var url in urlListData)
            {
                ...
                DateTime lastCheck = url.LastCheck;

                // son check edildiği tarihe göre işlem yaptırma.
                if (lastCheck == null || lastCheck.AddMinutes(Convert.ToInt32(url.Time)) <= DateTime.Now)
                {
                    url.LastCheck = DateTime.Now;
                    db.URLList.Update(url);
                    db.SaveChanges();

                    // hata döndüğünde mail göndere işlemeleri.
                    if (statusCode != 200)
                    {
                        var userData = db.Users.Where(q => q.Id == Convert.ToInt32(url.CreateAddById)).FirstOrDefault();
                        Tools tls = new Tools();
                        ...
                        {
                            //hata kaydını loglama işlemleri
